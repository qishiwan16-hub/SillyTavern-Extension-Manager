...
- 前端启停按扩展能力分层处理：支持标准 `hooks.enable/disable`、纯 CSS、显式清理函数或专用适配的扩展直接热切换；没有卸载生命周期的旧式扩展保存状态后自动刷新一次，确保事件、定时器和入口真实加载或停止。
...
- 可多选前端扩展进行分组、检测、顺序更新以及批量启用或禁用；整批状态会先全部保存，若含旧式扩展只在完成后刷新一次。
