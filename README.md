...
每次保存会先写临时文件再替换主文件，并把上一次数据保留为 `.bak`。前端扩展资料保存在 `extensions`，后端插件资料保存在 `backendPlugins`，悬浮球大小保存在 `settings.floatingBallSize`。日间/夜间偏好保存在浏览器本地，下次打开面板时自动恢复。也可以通过 `EXTENSION_MANAGER_DATA_DIR` 环境变量指定其他数据目录。
