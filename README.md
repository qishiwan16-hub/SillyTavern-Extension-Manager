...
- 前端、后端、白名单和检测结果页支持选择排序：首字母、安装/更新时间、启用状态、类型和检测状态；检测结果默认失败红色、可更新绿色、最新排序。
- 设置中提供“启用扩展优先”开关，开启后同一状态组内已启用扩展排在已禁用扩展前面。
- 后端管理页顶部单独提供扩展管理器后端检测与更新；普通批量检测和更新只处理其他后端插件，更新后提示手动重启 SillyTavern。安装或更新后可在安装页点击“连接后端/重新连接”读取状态和插件列表，无需刷新网页；后端代码要真正生效仍需重启酒馆进程。
- 前端扩展列表上方集中提供本体检测、扩展检测和更新操作，检测失败项可以一键重试。
- 前端列表优先复用 SillyTavern 已加载的扩展清单，并与管理后端资料并行读取；列表会先显示，中文标注和分组在后端返回后自动补齐。
...
- 前端扩展不需要自带热启停接口：扩展管理器会主动托管第三方扩展的原生/jQuery 事件、酒馆事件订阅、定时任务、动画帧、页面观察器、样式和界面入口，禁用时暂停，启用时原地恢复，全程不刷新浏览器。
- 热启停会隔离已经销毁的 iframe、观察器或特殊浏览器对象；单个资源清理失败不会阻止其他资源和插件入口继续禁用。
- 热启停只在扩展初始化或动态加载时识别插件资源；用户开始触摸或输入后停止被动识别，正常聊天、键盘弹出和页面操作不解析调用栈。事件、定时器和动画清理使用直接索引，不遍历全部历史资源。
- 禁用入口检查只在点击禁用时立即执行一次；不会延迟复查，也不会在重新读取列表时扫描全部禁用插件。
...
- 可多选前端扩展进行分组、检测、顺序更新以及批量启用或禁用；单项、多选、检测结果页和白名单页全部使用无刷新热启停。
...
- Ny 字体管理器具有专用热启停适配：禁用会清理字体效果、动态样式和设置入口，启用会恢复原字体开关、CSS、设置界面与聊天字体扫描。
...
重新启动 SillyTavern。以后安装或更新后端后，打开“安装扩展”页点击“连接后端/重新连接”即可重新读取状态，不需要刷新整个网页；若更新的是后端代码，首次生效仍必须重启酒馆进程。浏览器访问下面的地址进行验证：
