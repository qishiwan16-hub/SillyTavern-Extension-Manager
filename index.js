...
(function () {
    ...
    const SEAMLESS_RELOAD_KEY = 'st-extension-manager-seamless-reload-v1';
    const SEAMLESS_RELOAD_PARAM = 'st_hot_restart';

    function consumeSeamlessReloadNotice() {
        let notice = null;
        try {
            const currentUrl = new URL(location.href);
            const token = currentUrl.searchParams.get(SEAMLESS_RELOAD_PARAM);
            if (!token) return;
            currentUrl.searchParams.delete(SEAMLESS_RELOAD_PARAM);
            history.replaceState(history.state, document.title, currentUrl.href);
            notice = JSON.parse(sessionStorage.getItem(SEAMLESS_RELOAD_KEY) || 'null');
            sessionStorage.removeItem(SEAMLESS_RELOAD_KEY);
        } catch (error) {}
        if (notice) setTimeout(() => window.toastr?.info?.(notice.message || '页面已无感刷新，扩展状态已重新读取'), 1200);
    }

    consumeSeamlessReloadNotice();
    ...
    async function shouldUseSeamlessReload(extension) {
        ...
        try {
            const response = await fetch(extensionFileUrl(extension, js), { cache: 'no-store', credentials: 'same-origin' });
            if (!response.ok) return false;
            const source = await response.text();
            return /(?:import\s+(?:[^'"]+?from\s*)?|import\s*\(|(?:Shared)?Worker\s*\()\s*['"](?:\.\.?\/)/.test(source)
                || /new\s+URL\s*\(\s*['"]\.\.?\//.test(source);
        } catch (error) {
            return false;
        }
    }

    function requestSeamlessReload(extension, reason = '扩展更新需要重新建立页面运行时') {
        const token = Date.now() + '-' + Math.random().toString(36).slice(2);
        try {
            sessionStorage.setItem(SEAMLESS_RELOAD_KEY, JSON.stringify({
                folder: folderOf(extension),
                message: (extension.displayName || folderOf(extension)) + ' 已更新，页面已无感刷新并重新读取状态',
                reason,
                time: Date.now(),
            }));
            const currentUrl = new URL(location.href);
            currentUrl.searchParams.set(SEAMLESS_RELOAD_PARAM, token);
            location.replace(currentUrl.href);
        } catch (error) {
            throw new Error('无感刷新启动失败：' + (error.message || error));
        }
        return new Promise(() => {});
    }

    function generationLooksActive() {
        const selectors = ['#stop_generation', '#mes_stop', '#stop_button', '.mes_stop', '[data-i18n="Stop Generation"]'];
        return selectors.some(selector => Array.from(document.querySelectorAll(selector)).some(element => {
            const style = window.getComputedStyle?.(element);
            return !element.hidden && element.getAttribute('aria-hidden') !== 'true' && style?.display !== 'none' && style?.visibility !== 'hidden' && (element.offsetWidth > 0 || element.offsetHeight > 0);
        }));
    }

    async function waitForGenerationIdle(timeout = 180000) {
        if (!generationLooksActive()) return true;
        window.toastr?.warning?.('检测到正在生成，请先暂停生成；生成完成后会继续无感刷新。');
        return new Promise(resolve => {
            ...
            const timer = setInterval(() => {
                if (!generationLooksActive()) {
                    ...
                    clearInterval(timer);
                    window.toastr?.warning?.('生成尚未结束，已取消本次无感刷新；请空闲后再点击无感刷新。');
                    resolve(false);
                }
            }, 500);
        });
    ...
    async function runManualSeamlessReload($button) {
        $button.prop('disabled', true).html('<i class="fa-solid fa-spinner fa-spin"></i> 等待生成结束');
        try {
            const ready = await waitForGenerationIdle();
            if (!ready) return;
            await requestSeamlessReload({ displayName: '当前页面', id: 'manual-seamless-reload' }, '用户手动触发无感刷新');
        } finally {
            $button.prop('disabled', false).html('<i class="fa-solid fa-arrows-rotate"></i> 无感刷新');
        }
    }
    ...
    function cleanupPersonaWeaver() {
        ...
        // .persona_controls_buttons_block belongs to SillyTavern; only remove Persona Weaver's own nodes.
        removeKnownExtensionNodes(['#pw_persona_tool_btn', '#pw-style-link', '#pw-custom-style', '#pw-float-quote-btn', '.pw-float-quote-btn', '.pw-wrapper']);
    }
        ...
        try {
            ...
            if (data?.isUpToDate) {
                ...
                if (isSelf) await hotReloadSelf();
                    ...
                    const seamless = await shouldUseSeamlessReload(extension);
                    let reloadHandled = false;
                    if (seamless) {
                        const ready = await waitForGenerationIdle();
                        if (ready) {
                            await requestSeamlessReload(extension, '检测到扩展引用本地模块或 Worker');
                        }
                        reloadHandled = true;
                        if (!ready && window.toastr) toastr.warning(extension.displayName + ' 文件已更新，生成结束后请点击设置里的“无感刷新”');
                    }
                    if (!reloadHandled) {
                        try {
                            await hotReload(extension);
                        } catch (hotError) {
                            console.warn('[Extension Manager] Direct hot reload failed; falling back to seamless reload.', folder, hotError);
                            const ready = await waitForGenerationIdle();
                            if (ready) await requestSeamlessReload(extension, '直接热更新失败');
                            else if (window.toastr) toastr.warning(extension.displayName + ' 文件已更新，生成结束后请点击设置里的“无感刷新”');
                            reloadHandled = true;
                        }
                    }
                }
    ...
    async function showPopup() {
        ...
        $popup.on('click', '.em-seamless-refresh', function () { void runManualSeamlessReload($(this)); });
